...
def _render_trajectory_video(
    ...
    with ExitStack() as stack:
        ...
        print(f"train_cameras: {train_cameras}")
            ...
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                if output_format == "images":
                    ...
                    if image_format == "jpeg":
                        media.write_image(
                            output_image_dir / filenames[camera_idx], render_image, fmt="jpeg", quality=jpeg_quality
                        )
                if output_format == "video":
                    if writer is None:
...
class RenderCameraPath(BaseRender):
    ...
    def main(self) -> None:
        ...
        print("filepath: ", self.camera_path_filename)
        with open(self.camera_path_filename, "r", encoding="utf-8") as f:
            camera_path = json.load(f)
        seconds = camera_path["seconds"]
        
        print(f"\nseconds: {seconds}\n")
        print(camera_path["camera_path"][0])
        print("\n")
        
        crop_data = get_crop_from_json(camera_path)
        camera_path, filenames = get_path_from_json(camera_path)
        print(f"\nfilenames: {filenames}\n")
        ...
        print(f"\nrender names: {self.rendered_output_names}\n")
        print("output_filename: ", self.output_path)
...
class DatasetRender(BaseRender):
    ...
    def main(self):
        ...
        CONSOLE.print("[bold green]Rendering dataset")
        ...
        for split in self.split.split("+"):
            ...
            if split == "train":
                ...
                # CONSOLE.print(f"\n****dataparser_outputs:\n{dataparser_outputs}")
                # CONSOLE.print(f"\n****camera_to_worlds:\n{dataset.cameras.camera_to_worlds}")
            ...
            with Progress(
                ...
                for camera_idx, (camera, batch) in enumerate(progress.track(dataloader, total=len(dataset))):
                    # CONSOLE.print("camera_idx: ", camera_idx)
                    # CONSOLE.print("camera: ", camera)
                    with torch.no_grad():
                        outputs = pipeline.model.get_outputs_for_camera(camera) # ***********this is where all outputs get generated
                        # CONSOLE.print(f"\n****outputs:\n{outputs}")
                    ...
                    for rendered_output_name in rendered_output_names:
                        ...
                        if is_raw:
                            ...
                            # raw_depth = output_image
                            # raw_depth_output_dir = self.output_path / split / "raw-depth" / image_name 
                            # raw_depth_output_dir.parent.mkdir(exist_ok=True, parents=True)
                            # np.save(raw_depth_output_dir.with_suffix(".npy"), raw_depth)
